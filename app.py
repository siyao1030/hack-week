...
db = connection[db_name] # or connection.db_name
...
def ratings(major, number):
    if request.method == 'POST':
        ...
        return redirect(url_for('ratings', major = major, number = number), code=303)
            # redirect to the url for the ratings function and pass the arguments
            # and change post to get
            # 200 ok, 404 page not found, 303 see other page?

    reviews = list(db.reviews.find({'major':major, 'course_number': number}))
    course = db.course_info.find_one({'major':major, 'course_number':number})
    #findOne ends after one item has been found
...
def classes_for_major():
    ...
    school = request.args.get('school', 'any') # any-> get everything
    ...
    courses = sorted(list(db.course_info.find(filter_doc)), key=by_name) #call to db
